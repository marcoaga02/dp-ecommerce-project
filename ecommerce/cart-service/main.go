...
import (
	...
	"github.com/marcoaga02/dp-ecommerce-project/ecommerce/cart-service/internal"
...
func main() {
	...
	myLogger := logger.NewStdLogger(logLevel, "cart-service/main")
	...
	gormCartRepo := repository.NewGormCartRepository(db, logger.NewStdLogger(logLevel, "cart-service/db"))
	cartServer := internal.NewCartServer(gormCartRepo, logger.NewStdLogger(logLevel, "cart-service/server"))
	...
	healthServer.SetServingStatus("cart.CartService", healthpb.HealthCheckResponse_SERVING)
	healthpb.RegisterHealthServer(grpcServer, healthServer)

	myLogger.Info("Cart service listening on port %s", port)

	if err := grpcServer.Serve(lis); err != nil {
		myLogger.Fatal("gRPC cart server failed: %v", err)
	}
}
